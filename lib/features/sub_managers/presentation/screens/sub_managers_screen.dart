...
class _SubManagersBodyState extends State<_SubManagersBody> {
      ...
      builder: (ctx, state) {
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              _Header(count: subManagers.length, canAdd: canAdd, onAdd: () => _showForm(ctx, catalog: catalog)),
              const SizedBox(height: 16),
              if (state is SubManagerError && catalog.isEmpty)
                SubManagerRetryBanner(message: state.message, onRetry: () => ctx.read<SubManagerCubit>().load()),
              Expanded(
                child: isLoading
                    ? const Center(child: CircularProgressIndicator())
                    : subManagers.isEmpty
                        ? SubManagerEmptyState(onAdd: canAdd ? () => _showForm(ctx, catalog: catalog) : null)
                        : _SubManagerList(
                            subManagers: subManagers,
                            catalog: catalog,
                            isSubmitting: isSubmitting,
                            onEditInfo: (sm) => _showForm(ctx, catalog: catalog, subManager: sm),
                            onEditPermissions: (sm) => _showPermissions(ctx, catalog: catalog, subManager: sm),
  ...
  static List<SubManagerModel> _listOf(SubManagerState state) => switch (state) {
  ...
  static List<PermissionOption> _catalogOf(SubManagerState state) => switch (state) {
  ...
  void _showPermissions(BuildContext context, {required List<PermissionOption> catalog, required SubManagerModel subManager}) {
    showDialog(
      ...
      builder: (_) => BlocProvider.value(
        value: context.read<SubManagerCubit>(),
        child: SubManagerPermissionsDialog(catalog: catalog, subManager: subManager),
      ),
    );
  }
...
class _Header extends StatelessWidget {
  ...
  const _Header({required this.count, required this.canAdd, required this.onAdd});
      ...
      children: [
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('المدراء الفرعيون', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
              const SizedBox(height: 3),
              Text('$count مدير فرعي', style: const TextStyle(fontSize: 13, color: AppColors.textSecondary)),
            ],
          ),
        ),
        AppButton(label: 'إضافة مدير فرعي', icon: Icons.add, width: 170, height: 40, onPressed: canAdd ? onAdd : null),
      ],
    );
  }
}
